# PR-Reviewer — Project Overview

> **One-Line Pitch:** B2B SaaS that provides AI-driven code reviews and security audits directly in GitHub Pull Requests, specializing in Sovereign Matrix compliance.

| Field | Value |
|-------|-------|
| **Status** | ✅ Infrastructure Built |
| **Repo** | `e:\hermes\github_fleet\pr-reviewer` |
| **Price** | $15/seat/month |
| **Last Updated** | 2026-04-26 |

## Key Files
- `api/github_webhook.ts` — Pull Request event listener
- `api/ai_review_engine.ts` — RAG engine analyzing code diffs for security/performance
- `.sovereign_memory.md` — Architectural log
